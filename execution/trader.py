...
class TraderAPI(object):
    ...
    def _get_position(self):
        ...
        txtdata = self.s.get(vs.BASE_URL % (vs.P_TYPE['https'], 
                                            ...
                                            vs.PAGES['position']))
        jsonobj = utils.get_jdata(txtdata)
        df = pd.DataFrame(jsonobj['done_data'], columns=vs.POSITION_COLS)
    ...
    def entrust_list(self):
       ...
       state:状态
        ...
        txtdata = self.s.get(vs.ENTRUST_LIST_URL % (vs.P_TYPE['https'], 
                                            ...
                                            utils.nowtime_str()))
        jsonobj = utils.get_jdata(txtdata)
        df = pd.DataFrame(jsonobj['done_data'], columns=vs.ENTRUST_LIST_COLS)
    ...
    获取成交列表
        ...
        txtdata = self.s.get(vs.DEAL_LIST_URL % (vs.P_TYPE['https'], 
                                                 ...
                                                 utils.nowtime_str()))
        jsonobj = utils.get_jdata(txtdata)
        df = pd.DataFrame(jsonobj['done_data'], columns=vs.DEAL_LIST_COLS)
    ...
    def _get_baseinfo(self):
        ...
        jsonobj = utils.get_jdata(txtdata)
        stkdata = jsonobj['done_data']['moneytype0']
